...
Декораторы будут применяться, начиная с 
ближайшего к определению, в данном случае 
@make_split.
Множественные декораторы
Сначала мы разбиваем предложение на список слов и 
затем подсчитаем полученный список. В нашем предложении 2
слова.
Инвертирование декораторов приведет к подсчету 
символов в строке, преобразовать их в текст и 
разделит этот текст на список, вернув ['12'].
...
Если декорируемая функция имеет параметры 
мы должны включить их во внутреннюю функцию декоратора
и включить их в вызов func.
Использование аргументов функции
Мы можем оперировать или не оперировать этими аргументами
...
Использование аргументов декоратора
Нам необходимо обернуть еще одну функцию 
для хранения аргументов декоратора 
декоратора.
Использование аргументов декоратора
И вернуть внутренний декоратор.
Теперь мы можем указать, какие из 
аргументов ключевого слова функции мы 
следует писать заглавными буквами
