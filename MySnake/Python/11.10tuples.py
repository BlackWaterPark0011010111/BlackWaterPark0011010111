...
# смотрим сколько елементов в списке
print(len(tuple3))
...
# один за другим элементы Apples, Cherriesи Strawberries. Прокрутите список fruitsи выведите каждый элемент на экран.
...
Кортежи подобны спискам, но они не могут быть 
изменять. 
Они являются эквивалентом констант для 
коллекций и работают быстрее, чем списки.
Они:
- Имеют порядок.
...
кортеж с помощью конструктора tuple
конструктора.
Конструктор кортежей также может быть использован 
использовать для создания пустых кортежей, если не задан аргумент 
аргумента.
Результат будет таким же, как и при использовании 
пустые скобки ()
...
Кортежи могут содержать значения любого 
типа.
Элементы кортежа могут быть 
смешанных типов.
...
Печать кортежа покажет его значения 
в том же порядке, который использовался при 
при определении кортежа.
К каждому значению кортежа можно 
получить доступ к каждому значению кортежа, используя его числовую позицию
в кортеже (начиная с нуля). Эта позиция 
называется индексом.
Нарезка может быть использована для доступа к частям 
кортежа или изменить его порядок.
...
Кортежи также имеют метод index
который возвращает индекс 
первого вхождения заданного значения 
в кортеже.
Количество вхождений значения 
можно также получить с помощью метода count
метод
...
>
Элементы внутри кортежа не могут быть 
изменять.
Если элемент кортежа относится к другому типу 
итерируемого, то его содержимое все равно может быть изменено 
изменить
days[0] = "Sunday"
...
В отличие от списков, кортежи не могут быть 
изменить.
Поэтому они не имеют ни одного из 
методов, которые могут быть использованы в списках для 
манипулирования их содержимым:
- добавить  Append
- удалитьPop
- расширить
- вставитьInsert
...
Сравнение двух кортежей вернет значение True
если следующие утверждения истинны для 
элементов в нем:
- Они одинаковые
...
Кортежи имеют только методы для поиска 
и анализа значений:
- Index
- Count
